...
use crate::{
    ...
    splitstream::{
        DigestMap, EnsureObjectMessages, SplitStreamReader, SplitStreamWriter, WriterMessages,
    },
...
impl Repository {
    ...
    pub fn ensure_object(&self, data: &[u8]) -> Result<Sha256HashValue> {
        ...
        // ensure_dir("objects")?;
        mkdirat(rep.try_clone()?, "objects", 0o755.into()).or_else(|e| match e {
            Errno::EXIST => Ok(()),
            _ => Err(e),
        })?;

        // ensure_dir(&dir)?;
        mkdirat(rep.try_clone()?, &dir, 0o755.into()).or_else(|e| match e {
            Errno::EXIST => Ok(()),
            _ => Err(e),
        })?;
    ...
    pub fn ensure_symlink_new<P: AsRef<Path>>(name: P, target: &str, repository: Arc<Mutex<OwnedFd>>) -> ErrnoResult<()> {
